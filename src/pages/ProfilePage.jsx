      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-slate-800/50 rounded-2xl p-8 border border-slate-700 mb-8">
          <div className="flex items-center gap-6 mb-8">
            <div className="w-24 h-24 bg-emerald-600 rounded-2xl flex items-center justify-center text-white text-4xl font-bold">
            ...
            <div>
              ...
              <p className="text-emerald-400">Section: {user.sectionNumber}</p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-700/50 rounded-xl p-5 text-center">
              ...
              <p className="text-4xl font-bold text-emerald-400">{completedWeeks}/{totalWeeks}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 rounded-2xl p-8 border border-slate-700">
          ...
          <div className="space-y-3">
            {Array.from({ length: 12 }, (_, i) => i + 1).map((week) => {
              ...
              return (
                <div key={week} className={`flex items-center justify-between p-4 rounded-xl ${isCompleted ? 'bg-emerald-900/30 border border-emerald-500/30' : 'bg-slate-700/30'}`}>
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isCompleted ? 'bg-emerald-600' : 'bg-slate-600'}`}>
                      {isCompleted ? '✓' : week}
                    </div>
                    <span className="text-white">Week {week}</span>
                  </div>
                  {isCompleted ? (
                    <span className={`font-bold ${score >= 90 ? 'text-emerald-400' : score >= 75 ? 'text-blue-400' : score >= 60 ? 'text-yellow-400' : 'text-red-400'}`}>{score}%</span>
                  ) : (
