...
const AdminDashboard = ({ onLogout }) => {
  ...
  const filteredStudents = filter === 'all' 
    ? students 
    : students.filter(s => s.section_number === filter);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700">
            ...
            <p className="text-4xl font-bold text-emerald-400">{sections.length}</p>
          </div>
          <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700">
            <h3 className="text-slate-400 text-sm mb-2">Completed Course</h3>
            <p className="text-4xl font-bold text-blue-400">
              {students.filter(s => Object.values(s.progress || {}).filter(p => p.completed).length >= 12).length}
            </p>
          </div>
          <div className="bg-slate-800/50 rounded-2xl p-6 border border-slate-700">
            ...
            <p className="text-4xl font-bold text-yellow-400">
              {students.length > 0 
                ? Math.round(students.reduce((sum, s) => sum + calculateGrade(s.progress).avg, 0) / students.length)
                : 0}%
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden">
          <div className="p-6 border-b border-slate-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={fetchStudents} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg">
                Refresh
              </button>
            </div>
          </div>
                  ...
                  {filteredStudents.map((student) => {
                    ...
                    return (
                      <tr key={student.id} className="hover:bg-slate-700/30">
                        ...
                        <td className="px-6 py-4">
                          <span className="px-3 py-1 bg-emerald-600/20 text-emerald-400 rounded-full text-sm">{student.section_number}</span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-24 h-2 bg-slate-700 rounded-full overflow-hidden">
                              <div className="h-full bg-emerald-500" style={{ width: `${(completedWeeks / 12) * 100}%` }} />
                            </div>
